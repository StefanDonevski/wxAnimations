...
void ChartControl::OnPaint(wxPaintEvent& evt)
{
    ...
    if (gc && values.size() > 0)
    {
        ...
        double lowValue = std::min_element(values.begin(), values.end(), [](const auto& a, const auto& b)
            { return a.second < b.second; })
            ->second;
        double highValue = std::max_element(values.begin(), values.end(), [](const auto& a, const auto& b)
            { return a.second < b.second; })
            ->second;
        ...
        {
            pointArray[i] = valueToChartArea.TransformPoint({ values[i].first, values[i].second });
        }
        ...
        auto highlightPointOnChart = valueToChartArea.TransformPoint({ highlightedPoint.first, highlightedPoint.second });
        gc->SetBrush(*wxRED_BRUSH);

        auto r = FromDIP(5);
        gc->DrawEllipse(highlightPointOnChart.m_x - r, highlightPointOnChart.m_y - r, r * 2, r * 2);

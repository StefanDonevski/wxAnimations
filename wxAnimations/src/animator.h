...
public:
	...
	const std::vector<AnimatedValue>& GetAnimatedValuses() const
	{
...
private:
	...
	void OnTimer(wxTimerEvent& evt)
	{
		...
		auto elapcedMs = std::chrono::duration_cast<std::chrono::milliseconds>(now - startTime).count();

		if (elapcedMs >= animationDurationMs)
		{
			Stop();
			return;
		}

		double tNorm = elapcedMs / animationDurationMs;
